    ...
    public partial class Usuarios : Form
    {
        ...
        private void lb_users_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (lb_users.SelectedIndex != -1)
            {
                ...
                gb_userData.Text = "Datos de " + (lb_users.SelectedItem as DataRowView)["user"];
        ...
        private void LoadAllUserData(DataRow data)
        {
            ...
            for(int i=0;i<clb_permissions.Items.Count;i++)
            {
                ...
                if(permissions.Contains(int.Parse(castedItem["id"].ToString())))
                {
        ...
        private void cb_employe_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                lbl_employeNameValue.Text = data["nombre"] + " " + data["apellido"];
        ...
        {
            return (from object checkedItem 
                    in clb_permissions.CheckedItems 
                    select checkedItem as DataRowView 
                    into castedItem 
                    select int.Parse(castedItem["id"].ToString())).Sum();
        }
